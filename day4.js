function passphraseValid(data) {
  const rows = data.split("\n"); // array of each row
  const numOfInvalid = rows.reduce((acc, row) => {
    // number of invalid rows
    const passwords = row.split(" "); // each word in the row

    const matches = passwords.filter((password, i) => {
      // how many matches there are
      const restOfPasswords = passwords.slice(++i, passwords.length); // all passwords after the one we are on
