function checksum(data) {
  ...
  const numsToSum = rows.map(rowNums => {
    const eachStringInRow = rowNums.split("\t");
    const eachNumInRow = eachStringInRow.map(num => Number(num));
    const maxNum = eachNumInRow.reduce((max, num) => {
      if (num > max) return num;
      else return max;
    }, eachNumInRow[0]);
    const minNum = eachNumInRow.reduce((min, num) => {
  ...
  return numsToSum.reduce((sum, num) => {
    return num + sum;
  }, 0);
}

function checkDivide(data) {
  const rows = data.split("\n");
  const numsToSum = rows.map(rowNums => {
    const eachStringInRow = rowNums.split("\t");
    const eachNumInRow = eachStringInRow.map(num => Number(num));
    return eachNumInRow.reduce((final, num) => {
      eachNumInRow.forEach(element => {
        if (element === num) return final;
        if (element % num === 0) final = element / num;
      });
      return final;
    }, 0);
  });
...
console.log(
  checkDivide(`515	912	619	2043	96	93	2242	1385	2110	860	2255	621	1480	118	1230	99
